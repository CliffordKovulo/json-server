const users = [
  ...
  // Function to filter active users, extract popular posts, and calculate average likes per user
  const filterActiveUsersAndAnalyze = (users) => {

    const specificDate = new Date("2024-05-16T00:00:00");
    const oneWeekAgo = new Date(specificDate);
    oneWeekAgo.setDate(specificDate.getDate() - 7);
    
    return users
      .map(user => {
        
        const activePosts = user.posts.filter(post => new Date(post.timestamp) >= oneWeekAgo);
       
        const popularPosts = activePosts.filter(post => post.likes >= 10);
        return {
          ...user,
          activePosts: popularPosts 
        };
      })
      .filter(user => user.activePosts.length > 0)
      .reduce((result, user) => {
        const totalLikes = user.activePosts.reduce((sum, post) => sum + post.likes, 0);
        result.totalUsers++;
        result.totalPosts += user.activePosts.length;
  ...
  const result = filterActiveUsersAndAnalyze(users);
  
  if (result.totalUsers > 0) {
    result.averageLikesPerUser = result.totalLikes / result.totalUsers;
  }
